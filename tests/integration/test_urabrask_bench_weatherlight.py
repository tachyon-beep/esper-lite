...
def _urza_without_benchmarks(tmp_path: Path, count: int = 3) -> UrzaLibrary:
    root = tmp_path / "urza"
    ...
    for i in range(count):
        ...
        d = BlueprintDescriptor(
            ...
            description="wl urabrask bench test",
        )
...
async def test_weatherlight_urabrask_bench_worker_attaches_benchmarks(
    ...
    monkeypatch.setenv("URABRASK_BENCH_INTERVAL_S", "1")
    monkeypatch.setenv("URABRASK_BENCH_TOPN", "2")
    monkeypatch.setenv("URABRASK_BENCH_TIMEOUT_MS", "0")

    urza = _urza_without_benchmarks(tmp_path, count=3)
    ...
    wl._urza_runtime = UrzaRuntime(urza)  # type: ignore[attr-defined]

    # Manually construct the bench worker from settings and run once
    assert wl._settings.urabrask_bench_enabled  # type: ignore[attr-defined]
    wl._urabrask_bench = __import__("esper.urabrask.bench_worker", fromlist=["UrabraskBenchWorker"]).UrabraskBenchWorker(  # type: ignore[attr-defined]
        urza,
        wl._urza_runtime,
        interval_s=1,
        topn=2,
    ...
    stats = wl._urabrask_bench.run_once()  # type: ignore[attr-defined]
    assert stats["processed"] >= 1
    # Verify extras were attached for at least one record
    attached = 0
    for rec in urza.list_all().values():
        if "benchmarks" in (rec.extras or {}):
            attached += 1
    assert attached >= 1

    # Weatherlight telemetry should expose bench metrics
