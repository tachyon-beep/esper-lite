...
from esper.karn import KarnCatalog, BlueprintDescriptor, BlueprintParameterBounds, BlueprintTier
from esper.urza import UrzaLibrary
from esper.tezzeret.compiler import TezzeretCompiler, CompileJobConfig
...
@pytest.mark.asyncio
async def test_tezzeret_saves_graph_metadata_extras(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    compiler = TezzeretCompiler(CompileJobConfig(artifact_dir=artifacts, use_cuda=False, max_retries=0))
    forge = TezzeretForge(catalog, urza, compiler, wal_path=tmp_path / "tezzeret_wal.json", compile_timeout_s=5.0)
...
def test_tamiyo_consumes_urza_graph_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    pkt = leyline_pb2.SystemStatePacket(version=1, current_epoch=1, training_run_id="run", packet_id=desc.blueprint_id)
    ...
    assert any(name.startswith("tamiyo.gnn.feature_coverage.edges.layer_connects") for name in names)
