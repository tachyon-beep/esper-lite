...
def test_optimizer_and_hazard_registry_integration(tmp_path: Path) -> None:
    ...
    cfg = TamiyoGraphBuilderConfig(
        ...
        activation_type_registry=EmbeddingRegistry(EmbeddingRegistryConfig(tmp_path / "act.json", 64)),
        optimizer_family_registry=EmbeddingRegistry(EmbeddingRegistryConfig(tmp_path / "opt.json", 16)),
    ...
    s = packet.seed_states.add(); s.seed_id = "s1"
