...
def populated_snapshot():
    ...
    snapshot = SanctumSnapshot(
        ...
        current_epoch=100,
        current_episode=359,  # Episode number for telemetry search
        max_epochs=1000,
        task_name="MNIST",
    )
...
async def test_update_snapshot_updates_display(populated_snapshot):
    ...
    async with app.run_test():
        ...
        # Verify episode number is present in first column (all rows show same episode)
        ...
        # First column is episode number (same for all rows)
        assert "359" in str(row0_data[0])  # current_episode from fixture
        assert "359" in str(row1_data[0])
        assert "359" in str(row2_data[0])
...
async def test_ab_test_cohort_styling(populated_snapshot):
    """A/B test cohort should show colored pip next to episode number."""
    app = EnvOverviewTestApp()
    async with app.run_test():
        widget = app.query_one(EnvOverview)
        widget.update_snapshot(populated_snapshot)
...
def test_env_overview_highlights_last_action_env():
    ...
    result = widget._format_env_id(env, episode=359, last_action_env_id=2, last_action_timestamp=now)
...
def test_env_overview_no_highlight_after_hysteresis():
    ...
    result = widget._format_env_id(env, episode=359, last_action_env_id=2, last_action_timestamp=old_timestamp)
