...
def test_interaction_bonus_is_the_scaffolding_additend() -> None:
    """The +1 counterfactual-scaffolding term is ``interaction_bonus`` (canonical)."""
    assert "interaction_bonus" in ADDITEND_SIGN_MAP
    assert "synergy_bonus" not in ADDITEND_SIGN_MAP
    c = _components(interaction_bonus=0.3)
    decomposed = decompose_additends(0.3, c)
    assert decomposed["interaction_bonus"] == pytest.approx(0.3)
    assert decomposed["residual"] == pytest.approx(0.0)
