...
def test_bsds_enums_present() -> None:
    ...
    assert (
        leyline_pb2.HandlingClass.Value("HANDLING_CLASS_RESTRICTED")
        == leyline_pb2.HANDLING_CLASS_RESTRICTED
    )
    assert (
        leyline_pb2.ResourceProfile.Value("RESOURCE_PROFILE_GPU")
        == leyline_pb2.RESOURCE_PROFILE_GPU
    )
...
def test_benchmark_messages_present() -> None:
    ...
    bench.profiles.add(
        ...
        p50_latency_ms=12.3,
        p95_latency_ms=20.5,
        throughput_samples_per_s=1500.0,
    )
