...
export function createDecisionSnapshot(
  ...
  const defaults: DecisionSnapshot = {
    ...
    expected_value: 0.2,
    actual_reward: 0.1,
    alternatives: [['WAIT', 0.8], ['GERMINATE', 0.2]],
    action_success: true,
...
export function createTamiyoState(overrides: Partial<TamiyoState> = {}): TamiyoState {
  const defaults: TamiyoState = {
    ...
    ev_main: null,
    ev_cf: null,
    return_var_cf_share: null,
    return_var_main_share: null,
    return_var_residual_share: null,
    hra_leg_active: false,
    rvt_leg_active: false,
    advantage_per_head_normalized: false,
    advantage_norm_fellback_count: 0,
    min_sparse_head_advantage_std: 0,
    policy_loss: 0.05,
    value_loss: 0.1,
    entropy_loss: -0.02,
    grad_norm: 1.5,
    ...
    advantage_mean: 0,
    advantage_std: 1,
    advantage_skewness: 0,
    advantage_kurtosis: 0,
    advantage_min: -2,
    advantage_max: 2,
    advantage_raw_mean: 0,
    advantage_raw_std: 1,
    ...
    lstm_has_nan: false,
    lstm_has_inf: false,
    rollout_lstm_h_l2_total: null,
    rollout_lstm_c_l2_total: null,
    rollout_lstm_h_rms: null,
    rollout_lstm_c_rms: null,
    rollout_lstm_h_env_rms_mean: null,
    rollout_lstm_h_env_rms_max: null,
    rollout_lstm_c_env_rms_mean: null,
    rollout_lstm_c_env_rms_max: null,
    rollout_lstm_h_max: null,
    rollout_lstm_c_max: null,
    rollout_lstm_has_nan: false,
    ...
    head_q_grad_norm: 0.0,
    head_slot_grad_norm_prev: 0.7,
    head_blueprint_grad_norm_prev: 0.8,
    head_style_grad_norm_prev: 0.6,
    head_tempo_grad_norm_prev: 0.75,
    head_alpha_target_grad_norm_prev: 0.65,
    head_alpha_speed_grad_norm_prev: 0.7,
    head_alpha_curve_grad_norm_prev: 0.68,
    head_op_grad_norm_prev: 0.72,
    head_q_grad_norm_prev: 0.0,
    ...
    ev_main_history: [],
    ev_cf_history: [],
    cf_value_loss_history: [],
    return_var_cf_share_history: [],
    inner_epoch: 3,
    ppo_batch: 10,
    action_counts: { OBSERVE: 50, GERMINATE: 10 },
    total_actions: 60,
    cumulative_action_counts: { WAIT: 40, GERMINATE: 12, PRUNE: 5, FOSSILIZE: 3 },
    ...
    collapse_risk_score: 0.2,
    _previous_risk: 0.1,
    entropy_clip_correlation: -0.1,
    value_mean: 0.1,
    value_std: 0.2,
    value_min: -0.2,
    value_max: 0.4,
    initial_value_spread: 0.6,
...
export function createSnapshot(overrides: Partial<SanctumSnapshot> = {}): SanctumSnapshot {
  const defaults: SanctumSnapshot = {
    ...
    seed_lifecycle: {
      germination_count: 12,
      prune_count: 5,
      fossilize_count: 10,
      active_count: 3,
      total_slots: 4,
      germination_rate: 1.2,
      prune_rate: 0.5,
      fossilize_rate: 1.0,
      blend_success_rate: 0.66,
      avg_lifespan_epochs: 25,
