...
describe('CohortComparisonPanel', () => {
  ...
  it('marks a cohort blocked when recent telemetry contains numerical instability', () => {
    const wrapper = mount(CohortComparisonPanel, {
      props: {
        snapshotsByGroup: {
          A: createSnapshot({
            reward_mode: 'shaped',
            run_config: { ...createSnapshot().run_config, n_episodes: 100 },
            episode_stats: { ...createSnapshot().episode_stats, total_episodes: 4 },
            best_runs: [
              createBestRunRecord({
                final_accuracy: 0.2,
                growth_ratio: 1.12,
            ...
            event_log: [
              {
                ...
                env_id: null,
                message: 'Detected: NaN',
                episode: 4,
                relative_time: '(2s)',
                metadata: { detail: 'Detected: NaN' }
              }
            ]
          })
        },
    ...
    expect(wrapper.find('[data-testid="cohort-verdict"]').text()).toContain('blocked')
    expect(wrapper.find('[data-testid="cohort-summary"]').text()).toContain('cannot be used')
    expect(wrapper.find('[data-testid="cohort-B"]').classes()).toContain('blocked')
    expect(wrapper.find('[data-testid="cohort-B"]').classes()).not.toContain('leader')
    expect(wrapper.find('[data-testid="cohort-status-B"]').text()).toContain('Blocked')
    expect(wrapper.find('[data-testid="cohort-B"]').text()).toContain('unstable')
    expect(wrapper.find('[data-testid="cohort-B"]').text()).toContain('NaN')
  })

  it('excludes a blocked high-ROI cohort from the leader marker', () => {
    const wrapper = mount(CohortComparisonPanel, {
      props: {
        snapshotsByGroup: {
          A: createSnapshot({
    ...
    expect(wrapper.find('[data-testid="cohort-A"]').classes()).toContain('leader')
    expect(wrapper.find('[data-testid="cohort-B"]').classes()).not.toContain('leader')
    expect(wrapper.find('[data-testid="cohort-verdict"]').text()).toContain('blocked')
  })
})
