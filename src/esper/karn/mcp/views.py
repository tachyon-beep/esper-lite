...
VIEW_DEFINITIONS: dict[str, str] = {
    ...
    "runs": """
        ...
        SELECT
            ...
            json_extract_string(data, '$.reward_family') as reward_family,
            ...
            json_extract(data, '$.gamma')::DOUBLE as gamma,
            json_extract(data, '$.gae_lambda')::DOUBLE as gae_lambda,
            json_extract(data, '$.ppo_updates_per_batch')::INTEGER as ppo_updates_per_batch,
            json_extract(data, '$.recurrent_n_epochs')::INTEGER as recurrent_n_epochs,
            ...
            json_extract(data, '$.param_penalty_weight')::DOUBLE as param_penalty_weight,
            json_extract(data, '$.sparse_reward_scale')::DOUBLE as sparse_reward_scale,
            json_extract(data, '$.rent_host_params_floor')::INTEGER as rent_host_params_floor,
            json_extract(data, '$.basic_acc_delta_weight')::DOUBLE as basic_acc_delta_weight,
            json_extract(data, '$.per_head_advantage_norm')::BOOLEAN as per_head_advantage_norm,
            json_extract(data, '$.return_variance_telemetry')::BOOLEAN as return_variance_telemetry,
            json_extract(data, '$.value_coef')::DOUBLE as value_coef,
            json_extract(data, '$.value_warmup_batches')::INTEGER as value_warmup_batches,
            json_extract(data, '$.value_coef_start')::DOUBLE as value_coef_start,
            json_extract(data, '$.plateau_threshold')::DOUBLE as plateau_threshold,
            json_extract(data, '$.improvement_threshold')::DOUBLE as improvement_threshold,
            json_extract(data, '$.gradient_telemetry_stride')::INTEGER as gradient_telemetry_stride,
            json_extract(data, '$.lstm_hidden_dim')::INTEGER as lstm_hidden_dim,
            json_extract(data, '$.chunk_length')::INTEGER as chunk_length,
            json_extract(data, '$.max_seeds')::INTEGER as max_seeds,
            json_extract(data, '$.permissive_gates')::BOOLEAN as permissive_gates,
            json_extract(data, '$.auto_forward_g1')::BOOLEAN as auto_forward_g1,
            json_extract(data, '$.auto_forward_g2')::BOOLEAN as auto_forward_g2,
            json_extract(data, '$.auto_forward_g3')::BOOLEAN as auto_forward_g3,
            json_extract(data, '$.disable_pbrs')::BOOLEAN as disable_pbrs,
            json_extract(data, '$.disable_terminal_reward')::BOOLEAN as disable_terminal_reward,
            json_extract(data, '$.disable_anti_gaming')::BOOLEAN as disable_anti_gaming,
            json_extract(data, '$.max_grad_norm')::DOUBLE as max_grad_norm,
            json_extract(data, '$.slot_ids')::VARCHAR as slot_ids_json,
            json_extract(data, '$.env_devices')::VARCHAR as env_devices_json,
            json_extract_string(data, '$.resume_path') as resume_path,
            ...
            json_extract(data, '$.compile_enabled')::BOOLEAN as compile_enabled,
            json_extract_string(data, '$.compile_backend') as compile_backend,
            json_extract_string(data, '$.compile_mode') as compile_mode,
            json_extract(data, '$.host_params')::INTEGER as host_params,
            json_extract_string(data, '$.proof_profile') as proof_profile,
    ...
    "ppo_updates": """
        ...
        SELECT
            ...
            -- EV-stab Stage 0/2 per-stream EV + cf-head loss + gate metrics (NULL on legs
            -- that don't emit them). ev_sum == explained_variance on the ON leg.
            -- cov_rcf_return_share / r_main_cov are ON-leg (HRA) λ-return DIAGNOSTICS —
            -- V_cf-contaminated, NOT the gate (PDR-0028). The TRUE Stage-0 gate is the
            -- value-free return_var_cf_share (>0.40 => cf stream dominates Var[return]),
            -- read on the Stage-2-OFF control run; return_var_main_share is the smoothness
            -- leg. The gap (return_var_cf_share - cov_rcf_return_share) = V_cf contamination.
            -- Pure telemetry, never gate inputs.
