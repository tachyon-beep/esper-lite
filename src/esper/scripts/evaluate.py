...
Usage:
    python -m esper.scripts.evaluate --model models/ppo_tamiyo.pt --episodes 20
    python -m esper.scripts.evaluate --model models/ppo_tamiyo.pt --episodes 50 --verbose
...
def run_diagnostic_episode(
    ...
    from esper.simic.ppo import signals_to_features, safe
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        phase = classify_phase(epoch, max_epochs)
...
def compute_diagnostics(records: list[EpisodeRecord], max_epochs: int) -> DiagnosticReport:
    ...
    report.std_final_accuracy = (sum((a - report.mean_final_accuracy)**2 for a in accuracies) / len(accuracies)) ** 0.5
    ...
    report.action_frequencies = {k: v / total_actions for k, v in action_counts.items()} if total_actions > 0 else {}
    ...
    report.mean_seed_lifetime = sum(seed_lifetimes) / len(seed_lifetimes) if seed_lifetimes else 0
    ...
    for record in records:
        for i, step in enumerate(record.steps):
            future_return = sum(gamma**j * s.reward for j, s in enumerate(record.steps[i:]))
    ...
    if len(values) > 1:
        ...
        cov = sum((v - mean_v) * (r - mean_r) for v, r in zip(values, actual_returns)) / len(values)
        std_v = (sum((v - mean_v)**2 for v in values) / len(values)) ** 0.5
        std_r = (sum((r - mean_r)**2 for r in actual_returns) / len(actual_returns)) ** 0.5
        report.value_return_correlation = cov / (std_v * std_r) if std_v * std_r > 0 else 0
    ...
    if report.total_seeds_created > 0:
        report.seed_survival_rate = report.total_seeds_fossilized / report.total_seeds_created
    ...
    report.mean_accuracy_with_seeds = sum(acc_with_seeds) / len(acc_with_seeds) if acc_with_seeds else 0
    report.mean_accuracy_without_seeds = sum(acc_without_seeds) / len(acc_without_seeds) if acc_without_seeds else 0

    # Red flags
    max_freq = max(report.action_frequencies.values()) if report.action_frequencies else 0
    if max_freq > 0.9:
        dominant = max(report.action_frequencies, key=report.action_frequencies.get)
        report.red_flags.append(f"COLLAPSED POLICY: {dominant} chosen {max_freq*100:.1f}% of time")

    if report.mean_entropy < 0.1:
        report.red_flags.append(f"LOW ENTROPY: {report.mean_entropy:.3f} - policy may be too deterministic")
    elif report.mean_entropy > 1.8:
        # Max entropy for 7 actions is ln(7) ~ 1.95
        report.red_flags.append(f"HIGH ENTROPY: {report.mean_entropy:.3f} - policy may be near-random")

    if report.value_return_correlation < 0.5:
        report.red_flags.append(f"POOR VALUE CALIBRATION: R={report.value_return_correlation:.2f} - value function is not predictive")

    if report.total_seeds_created == 0:
        report.red_flags.append("NO SEEDS CREATED: Tamiyo never germinated")
    elif report.seed_survival_rate < 0.1:
        report.red_flags.append(f"LOW SEED SURVIVAL: {report.seed_survival_rate*100:.1f}% - seeds rarely reach fossilization")
...
def format_report(report: DiagnosticReport, verbose: bool = False) -> str:
    ...
    lines.append(f"Final accuracy: {report.mean_final_accuracy:.2f}% ± {report.std_final_accuracy:.2f}%")
    ...
    if report.action_by_phase:
        ...
        for phase in ['early', 'mid', 'late']:
            if phase in report.action_by_phase:
                top_actions = sorted(report.action_by_phase[phase].items(), key=lambda x: -x[1])[:3]
    ...
    lines.append("-" * 70)
    lines.append(f"  V(s) vs actual return correlation: {report.value_return_correlation:.3f}")
    lines.append(f"  Mean absolute error: {report.value_mae:.3f}")
    lines.append(f"  Bias (mean error): {report.value_bias:+.3f}")

    quality = "GOOD" if report.value_return_correlation > 0.7 else "FAIR" if report.value_return_correlation > 0.5 else "POOR"
    ...
    if report.action_by_loss_trend:
        ...
        for trend in ['improving', 'stagnating', 'worsening']:
            if trend in report.action_by_loss_trend:
                top_actions = sorted(report.action_by_loss_trend[trend].items(), key=lambda x: -x[1])[:3]
                top_str = ", ".join(f"{a}={p*100:.0f}%" for a, p in top_actions)
    ...
    if report.action_by_seed_state:
        ...
        for state in ['no_seed', 'has_seed']:
            if state in report.action_by_seed_state:
                top_actions = sorted(report.action_by_seed_state[state].items(), key=lambda x: -x[1])[:3]
                top_str = ", ".join(f"{a}={p*100:.0f}%" for a, p in top_actions)
    ...
    if report.mean_accuracy_with_seeds > 0 or report.mean_accuracy_without_seeds > 0:
        ...
        lines.append(f"  Mean accuracy without seeds: {report.mean_accuracy_without_seeds:.2f}%")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m esper.scripts.evaluate --model models/ppo.pt --episodes 20
  python -m esper.scripts.evaluate --model models/ppo.pt --episodes 50 --device cuda:0
        """
    )
    parser.add_argument("--model", type=str, required=True, help="Path to trained PPO model (.pt file)")
    ...
    parser.add_argument("--verbose", action="store_true", help="Print detailed per-episode info")
    ...
    for ep in range(args.episodes):
        ...
        if args.verbose:
            print(f"  Episode {ep+1}: acc={record.final_accuracy:.2f}%, return={record.total_return:.2f}, seeds={record.seeds_created}")
        else:
