...
def test_policy_raises_when_no_seed_candidates() -> None:
    policy = TamiyoPolicy(TamiyoPolicyConfig(enable_compile=False))
    packet = leyline_pb2.SystemStatePacket(version=1, training_run_id="run-no-seed", packet_id="pkt-no-seed")
    with pytest.raises(DependencyViolationError) as exc:
        policy.select_action(packet)
    message = str(exc.value)
    assert "no_seed_candidates" in message
    assert exc.value.context.get("training_run_id") == "run-no-seed"


def test_build_command_requires_seed_id() -> None:
    policy = TamiyoPolicy(TamiyoPolicyConfig(enable_compile=False))
    packet = leyline_pb2.SystemStatePacket(version=1, training_run_id="missing-seed")
    with pytest.raises(DependencyViolationError):
        policy._build_command(
            action_idx=0,
            param_delta=0.0,
            blending_method=policy._blending_methods[0],
            selected_seed="   ",
            selected_blueprint="bp-required",
            packet=packet,
...
def test_build_command_requires_blueprint_id() -> None:
    policy = TamiyoPolicy(TamiyoPolicyConfig(enable_compile=False))
    packet = leyline_pb2.SystemStatePacket(version=1, training_run_id="missing-blueprint")
    with pytest.raises(DependencyViolationError):
        policy._build_command(
            action_idx=0,
            param_delta=0.0,
            blending_method=policy._blending_methods[0],
            selected_seed="seed-required",
            selected_blueprint="   ",
            packet=packet,
            schedule_values=(0.0, 0.0),
            breaker_logits=None,
