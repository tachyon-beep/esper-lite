...
from esper.tamiyo import FieldReportStore, FieldReportStoreConfig, TamiyoService
...
class _PolicyStub:
    def __init__(self) -> None:
        self.architecture_version = "tamiyo_stub_v1"
        self.last_action = {
            "action": 0.0,
            "param_delta": 0.0,
            "value_estimate": 0.0,
    ...
    def select_action(self, packet: leyline_pb2.SystemStatePacket) -> leyline_pb2.AdaptationCommand:
        command = leyline_pb2.AdaptationCommand(
            ...
            command_id=f"cmd-{packet.current_epoch}",
        )
        if packet.seed_states:
            command.target_seed_id = packet.seed_states[0].seed_id
        self.last_action = {
            "action": 0.0,
            "param_delta": 0.0,
            "value_estimate": 0.0,
    ...
    def state_dict(self) -> dict[str, float]:  # pragma: no cover - minimal stub for tests
        return {}

    def load_state_dict(self, _state: dict[str, float]) -> None:  # pragma: no cover
        return
...
def test_observation_window_synthesises_after_n_epochs(
    ...
    service = TamiyoService(
        policy=_PolicyStub(), store_config=store_cfg, urza=urza, signature_context=_SIGN,
        step_timeout_ms=0.0,
        metadata_timeout_ms=0.0)
