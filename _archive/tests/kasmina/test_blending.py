...
from esper.kasmina.blending import AlphaBlender, AlphaSchedule, blend_convex
...
def test_alpha_schedule_progression() -> None:
    schedule = AlphaSchedule(total_steps=10, temperature=1.0)
    values = [schedule.value(step) for step in range(11)]
    assert values[0] < values[5] < values[-1]
    assert 0.0 <= values[0] <= 1.0
    assert 0.0 <= values[-1] <= 1.0


def test_blender_detaches_host_tensor() -> None:
    host = torch.tensor([[1.0]], requires_grad=True)
    seed = torch.tensor([[2.0]], requires_grad=True)
    out = blend_convex(host, seed, alpha=0.5)
    out.backward()
    assert host.grad is None
