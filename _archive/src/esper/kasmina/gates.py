...
class GateInputs:
    ...
    mesh_required_layers: tuple[str, ...] = field(default_factory=tuple)
    mesh_available_layers: tuple[str, ...] = field(default_factory=tuple)
...
class KasminaGates:
    ...
    def _evaluate_g3(self, inputs: GateInputs) -> GateResult:
        ...
        required = {layer.strip() for layer in inputs.mesh_required_layers if layer.strip()}
        if required:
            available = {
                layer.strip() for layer in inputs.mesh_available_layers if layer.strip()
            }
            ...
            if missing:
                return GateResult(
                    gate=leyline_pb2.SEED_GATE_G3_INTERFACE,
                    passed=False,
                    reason="mesh_incomplete",
                    attributes={
                        "missing_layers": ",".join(missing),
                        "required_layers": ",".join(sorted(required)),
                        "available_layers": ",".join(sorted(available)),
                    },
                )
        return GateResult(
            gate=leyline_pb2.SEED_GATE_G3_INTERFACE,
    ...
    def _evaluate_g4(self, inputs: GateInputs) -> GateResult:
        raw_status = (inputs.performance_status or "").strip()
        status = raw_status.lower()
        if status == "fallback":
            return GateResult(
                gate=leyline_pb2.SEED_GATE_G4_SYSTEM_IMPACT,
                passed=False,
                reason="fallback_kernel_active",
                attributes={"status": raw_status or "fallback"},
            )
        if status not in {"nominal", "healthy"}:
            return GateResult(
                gate=leyline_pb2.SEED_GATE_G4_SYSTEM_IMPACT,
                passed=False,
                reason=f"performance status {raw_status}",
                attributes={"status": raw_status},
            )
        if inputs.isolation_violations:
            return GateResult(
                gate=leyline_pb2.SEED_GATE_G4_SYSTEM_IMPACT,
