...
class EsperSettings(BaseSettings):
    ...
    oona_telemetry_stream: str = Field(alias="OONA_TELEMETRY_STREAM", default="oona.telemetry")
    oona_policy_stream: str = Field(alias="OONA_POLICY_STREAM", default="oona.policy")
    oona_emergency_max_per_min: int | None = Field(
        alias="OONA_EMERGENCY_MAX_PER_MIN", default=None
    )
    oona_emergency_threshold: int | None = Field(
        alias="OONA_EMERGENCY_THRESHOLD", default=None
    )
    oona_message_ttl_ms: int | None = Field(alias="OONA_MESSAGE_TTL_MS", default=900_000)
    kernel_freshness_window_ms: int = Field(alias="KERNEL_FRESHNESS_WINDOW_MS", default=60_000)
    ...
    elasticsearch_url: str = Field(alias="ELASTICSEARCH_URL", default="http://localhost:9200")
    ...
    tamiyo_field_report_max_retries: int = Field(alias="TAMIYO_FIELD_REPORT_MAX_RETRIES", default=3)
    tamiyo_wal_strict_validation: bool = Field(
        alias="TAMIYO_WAL_STRICT_VALIDATION", default=True
    )
    # Optional blend-mode annotations for Kasmina (P8)
    tamiyo_enable_blend_mode_ann: bool = Field(alias="TAMIYO_ENABLE_BLEND_MODE_ANN", default=False)
    tamiyo_blend_mode_default: str = Field(alias="TAMIYO_BLEND_MODE_DEFAULT", default="CONVEX")
    tamiyo_blend_conf_gate_k: float = Field(alias="TAMIYO_BLEND_CONF_GATE_K", default=1.0)
    tamiyo_blend_conf_gate_tau: float = Field(alias="TAMIYO_BLEND_CONF_GATE_TAU", default=1.0)
    tamiyo_blend_alpha_lo: float = Field(alias="TAMIYO_BLEND_ALPHA_LO", default=0.0)
    tamiyo_blend_alpha_hi: float = Field(alias="TAMIYO_BLEND_ALPHA_HI", default=1.0)
    tamiyo_blend_alpha_vec_max: int = Field(alias="TAMIYO_BLEND_ALPHA_VEC_MAX", default=64)
    # P9 — Field Report Lifecycle (observation windows + retry backoff)
    tamiyo_fr_obs_window_epochs: int = Field(alias="TAMIYO_FR_OBS_WINDOW_EPOCHS", default=1)
    tamiyo_fr_retry_backoff_ms: int = Field(alias="TAMIYO_FR_RETRY_BACKOFF_MS", default=1000)
    tamiyo_fr_retry_backoff_mult: float = Field(alias="TAMIYO_FR_RETRY_BACKOFF_MULT", default=2.0)
    # Policy update verification knobs
    tamiyo_verify_updates: bool = Field(alias="TAMIYO_VERIFY_UPDATES", default=False)
    tamiyo_update_freshness_sec: int = Field(alias="TAMIYO_UPDATE_FRESHNESS_SEC", default=0)
    ...
    tamiyo_step_timeout_ms: float = Field(alias="TAMIYO_STEP_TIMEOUT_MS", default=5.0)
    ...
    tamiyo_enable_compile: bool | None = Field(alias="TAMIYO_ENABLE_COMPILE", default=None)

    urza_database_url: str = Field(
        alias="URZA_DATABASE_URL", default="sqlite:///./var/urza/catalog.db"
    )
    ...
    tolaria_opt_rebuild_enabled: bool = Field(alias="TOLARIA_OPT_REBUILD_ENABLED", default=False)
    ...
    # Rollback (two-tier)
    tolaria_rollback_enabled: bool = Field(alias="TOLARIA_ROLLBACK_ENABLED", default=False)
    tolaria_rollback_fast_cap_mb: int = Field(alias="TOLARIA_ROLLBACK_FAST_CAP_MB", default=32)
    tolaria_rollback_deadline_ms: int = Field(alias="TOLARIA_ROLLBACK_DEADLINE_MS", default=250)
    tolaria_rollback_snapshot_steps: int = Field(alias="TOLARIA_ROLLBACK_SNAPSHOT_STEPS", default=1)

    # Emergency protocol
    tolaria_emergency_enabled: bool = Field(alias="TOLARIA_EMERGENCY_ENABLED", default=False)
    ...
    tolaria_pcgrad_enabled: bool = Field(alias="TOLARIA_PCGRAD_ENABLED", default=False)
    ...
    tolaria_agg_per_layer_topk: int = Field(alias="TOLARIA_AGG_PER_LAYER_TOPK", default=5)
    tolaria_seed_layer_summaries_enabled: bool = Field(
        alias="TOLARIA_SEED_LAYER_SUMMARIES_ENABLED", default=False
    )
    tolaria_seed_layer_topk: int = Field(alias="TOLARIA_SEED_LAYER_TOPK", default=3)
    # Per-seed telemetry thresholds
    tolaria_seed_share_jump_warn: float = Field(alias="TOLARIA_SEED_SHARE_JUMP_WARN", default=0.3)
    ...
    tolaria_seed_health_compact: bool = Field(alias="TOLARIA_SEED_HEALTH_COMPACT", default=False)
    ...
    # Profiler
    tolaria_profiler_enabled: bool = Field(alias="TOLARIA_PROFILER_ENABLED", default=False)
    tolaria_profiler_dir: str = Field(alias="TOLARIA_PROFILER_DIR", default="./var/profiler")
    tolaria_profiler_active_steps: int = Field(alias="TOLARIA_PROFILER_ACTIVE_STEPS", default=50)
    ...
    urabrask_oona_publish_enabled: bool = Field(
        alias="URABRASK_OONA_PUBLISH_ENABLED", default=False
    )
    ...
    urabrask_wal_path: str = Field(
        alias="URABRASK_WAL_PATH", default="./var/urza/urabrask_wal.jsonl"
    )
    ...
    # -----------------------------
    urabrask_crucible_allow_oom: bool = Field(alias="URABRASK_CRUCIBLE_ALLOW_OOM", default=False)
