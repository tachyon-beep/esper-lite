...
from esper.tamiyo.exceptions import TamiyoPersistenceError
...
class FieldReportStore:
    ...
    def __init__(self, config: FieldReportStoreConfig) -> None:
        ...
        self._reports: list[leyline_pb2.FieldReport] = list(
            self._load_from_disk(strict=self._config.strict_validation)
        )
    ...
    def load_errors(self) -> list[str]:
        """Return non-fatal load errors encountered during WAL parsing."""
    ...
    def _load_from_disk(self, *, strict: bool) -> Iterable[leyline_pb2.FieldReport]:
        ...
        with self._path.open("rb") as handle:
            while True:
                ...
                if len(length_bytes) != _LENGTH_STRUCT.size:
                    msg = "incomplete length prefix encountered in field report WAL"
                    if strict:
                        raise TamiyoPersistenceError(f"{msg} (path={self._path})")
                    LOGGER.warning(msg)
                ...
                if len(payload) != length:
                    msg = (
                        "field report WAL truncated" if len(payload) else "field report WAL missing payload"
                    )
                    if strict:
                        raise TamiyoPersistenceError(f"{msg} (path={self._path})")
                    LOGGER.warning(msg)
                ...
                try:
                    ...
                    msg = f"failed to parse field report entry (path={self._path}): {exc}"
                    if strict:
                        raise TamiyoPersistenceError(msg) from exc
                    LOGGER.warning(msg)
                    self._load_errors.append(msg)
                    break
...
def _fsync_directory(path: Path) -> None:
    """Fsync the directory containing the given path to ensure rename durability."""

    try:
        dir_fd = os.open(path.parent, os.O_DIRECTORY | os.O_RDONLY)  # type: ignore[arg-type]
    except (FileNotFoundError, NotADirectoryError):  # pragma: no cover - defensive
        return
    try:
        os.fsync(dir_fd)
