...
class TamiyoPolicy(nn.Module):
    ...
    def __init__(self, config: TamiyoPolicyConfig | None = None) -> None:
        ...
        if config is None:
            ...
            cfg = config
        self._config = cfg
        strict_env = os.getenv("TAMIYO_STRICT_COMMAND_IDS")
        if strict_env is not None:
            self._strict_command_ids = strict_env.lower() in {"1", "true", "yes", "on"}
        else:
            self._strict_command_ids = bool(cfg.strict_command_ids)
    ...
    def select_action(self, packet: leyline_pb2.SystemStatePacket) -> leyline_pb2.AdaptationCommand:
        ...
        # Fast-path: abort when no trustworthy seed candidates are available.
        if not seed_candidates:
            self._last_action = {
                "action": 2.0,
                "param_delta": 0.0,
                "policy_param_vector": (),
                "blending_method": self._blending_methods[0] if self._blending_methods else "",
                "blending_index": 0.0,
                "value_estimate": 0.0,
                "risk_index": 0.0,
                "risk_score": 0.0,
        ...
        if outputs is None:
            ...
            command.annotations["tamiyo_runtime_fallback"] = "true"
            logger.critical("Tamiyo degraded to runtime fallback pause command")
            self._last_action = {
        ...
        raw_blending_logits = outputs["blending_logits"].detach().cpu().view(-1)
        ...
        self._last_action = {
            ...
            "fallback_seed": 1.0 if command.annotations.get("tamiyo_fallback_seed") == "true" else 0.0,
            "fallback_blueprint": 1.0 if command.annotations.get("tamiyo_fallback_blueprint") == "true" else 0.0,
            "synthetic_pause": 1.0 if command.annotations.get("tamiyo_synthetic_pause") == "true" else 0.0,
            "runtime_fallback": 1.0 if command.annotations.get("tamiyo_runtime_fallback") == "true" else 0.0,
        }
        self._last_feature_coverage = coverage
        self._last_feature_coverage_types = coverage_types
    ...
    def _maybe_emit_blend_mode_annotations(
        ...
        confidence_logits: list[float] | None,
    ) -> None:
        ...
        if raw_method == "CONFIDENCE":
            ...
            if confidence_logits is not None:
                try:
                    ...
                    command.annotations["confidence_logits"] = _json.dumps([float(x) for x in confidence_logits])
                except Exception:
                    logger.warning("Failed to encode confidence logits for annotations")
        else:
            # Remove remnants if the policy previously set confidence mode but switched away.
            for key in ("gate_k", "gate_tau", "alpha_lo", "alpha_hi", "confidence_logits_required", "confidence_logits"):
    ...
    def _build_command(
        ...
        if action_idx == 0:  # seed graft
            ...
            seed_id = (selected_seed or "").strip()
            if not seed_id:
                raise DependencyViolationError(
                    "tamiyo",
                    "seed command missing target_seed_id",
                    context={
            ...
            command.target_seed_id = seed_id
            command.seed_operation.operation = leyline_pb2.SEED_OP_GERMINATE
            blueprint_id = (selected_blueprint or "").strip()
            if not blueprint_id:
                raise DependencyViolationError(
                    "tamiyo",
                    "seed command missing blueprint_id",
                    context={
                        "command_id": command.command_id or "",
                        "seed_id": seed_id,
                        "packet_id": packet.packet_id or "",
            ...
            command.annotations.setdefault("reason", "policy")
