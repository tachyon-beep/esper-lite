...
> **Latest fix:** TELE-340 (lstm_health) - wired into PPO update loop
...
|-------|----------|--------|------------|
| TELE-220 to TELE-228 | Value Function Metrics | 9 | Emitters not implemented |
...
## TELE-220 to TELE-228: Value Function Metrics

**File:** `tests/telemetry/test_tele_value_metrics.py`

These metrics exist in `ValueFunctionMetrics` schema but no emitter computes them.

| TELE | Metric | Gap | Fix Required |
|------|--------|-----|--------------|
| TELE-220 | `v_return_correlation` | No emitter | Compute correlation between value predictions and actual returns |
| TELE-221 | `td_error_mean` | No emitter | Compute mean TD error during PPO updates |
| TELE-222 | `td_error_std` | No emitter | Compute TD error standard deviation |
| TELE-223 | `bellman_error` | No emitter | Compute Bellman residual |
| TELE-224 | `return_p10` | No emitter | Compute 10th percentile of returns |
| TELE-225 | `return_p50` | No emitter | Compute median of returns |
| TELE-226 | `return_p90` | No emitter | Compute 90th percentile of returns |
| TELE-227 | `return_variance` | No emitter | Compute variance of returns |
| TELE-228 | `return_skewness` | No emitter | Compute skewness of returns |

**Implementation Location:** `src/esper/simic/telemetry/emitters.py` → `emit_ppo_update_event()`

**Data Source:** These should be computed from:
- `value_predictions` deque (predicted values)
- `actual_returns` deque (observed returns)
- Both maintained during rollout collection
...
| `test_tele_value_metrics.py` | TELE-220 to TELE-228 | 9 xfails (no emitters) |
