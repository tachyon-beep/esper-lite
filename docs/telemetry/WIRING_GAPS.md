...
> **Latest fix:** TELE-610 (episode stats) - wired into training loop through aggregator
...
## TELE-610: Episode Stats - FIXED ✓

**File:** `tests/telemetry/test_environment_metrics.py`

All episode statistics wiring gaps have been resolved:

> **Fixed (2026-01-04):**
> - TELE-610 `episode_stats.avg_length` - computed from episode lengths in batch
> - TELE-610 `episode_stats.success_rate` - computed from episode outcomes (success/total)
> - TELE-610 `episode_stats.avg_actions_per_episode` - computed from action count / episodes
> - TELE-610 `episode_stats.trend` - computed from recent episode length trend
>
> **Implementation:** `src/esper/simic/telemetry/emitters.py` → `emit_batch_completed_event()` emits `EpisodeStatsPayload`, aggregator stores in snapshot
