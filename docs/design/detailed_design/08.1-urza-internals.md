...
This document provides the complete internal implementation details of Urza, the central library for the Esper morphogenetic platform. Urza manages all architectural assets with immutable storage, comprehensive metadata, semantic versioning, and advanced query capabilities. The implementation includes critical C-016 fixes for production safety.

Key characteristics:
- **Content-Addressable Storage**: SHA-256 based immutable storage with integrity verification
- **Multi-Tier Caching**: Sub-10ms retrieval through L1-L4 cache hierarchy
...
├──────────────┼──────────────┼──────────────┼───────────────┤
│     Karn     │   Advanced   │   Object     │  Blueprint    │
│   Receiver   │   Query API  │    Store     │  Lifecycle    │
├──────────────┼──────────────┼──────────────┼───────────────┤
│   Tezzeret   │    Tamiyo    │  Metadata    │   Kernel      │
│   Receiver   │   Handler    │   Database   │  Lifecycle    │
├──────────────┼──────────────┼──────────────┼───────────────┤
│   Urabrask   │    Cache     │   Multi-     │  Retention    │
│   Processor  │   Manager    │  Tier Cache  │   Manager     │
└──────────────┴──────────────┴──────────────┴───────────────┘
```
...
class UrzaCentralLibrary:
    ...
    def __init__(self, config: UrzaConfig):
        ...
        self.karn_interface = KarnBlueprintReceiver()
        self.tezzeret_interface = TezzeretArtifactReceiver()
        self.urabrask_interface = UrabraskReportProcessor()
...
class BlueprintLifecycle:
    ...
    STATES = {
        'GENERATED': 'Created by Karn, awaiting compilation',
        'SUBMITTED': 'Submitted to Tezzeret for compilation',
        'COMPILING': 'Currently being compiled',
        'COMPILED': 'At least one artifact available',
        'VALIDATING': 'Artifacts undergoing Urabrask validation',
        'VALIDATED': 'Has validated artifacts',
...
class KernelLifecycle:
    """Manage kernel artifact states with memory leak prevention."""

    STATES = {
        'PENDING_BENCHMARKING': 'Awaiting Urabrask validation',
        'BENCHMARKING': 'Currently being validated',
    ...
    def __init__(self):
        ...
        self.artifact_cache: Dict[Tuple[str, int], KernelMetadata] = {}  # (id, epoch)
        self.cache_ttl_epochs = 100
        self.last_gc_epoch = 0

    def process_validation_report(
        self,
        report: UrabraskValidationReport
    ) -> KernelStateUpdate:
        """[C-016] Update kernel state with memory management."""

        # [C-016] Periodic garbage collection
        ...
        kernel_id = report.artifact_id

        if report.passed:
            new_state = 'VALIDATED'
            # Extract and store tags
            self.store_empirical_tags(kernel_id, report.generated_tags)
            # Store performance metrics
            self.store_performance_metrics(kernel_id, report.detailed_metrics)
            # Update risk assessment
            self.update_risk_assessment(kernel_id, report.risk_score)
        else:
            new_state = 'REJECTED'
            # Store rejection reason
            self.store_rejection_reason(kernel_id, report.failure_reason)

        return self.transition_state(kernel_id, new_state)
...
|-----------|----------|---------|
| Karn | BlueprintProto | Async/Queue |
| Tezzeret | KernelProto | Async/Queue |
| Urabrask | ValidationReport | Async/Queue |
