...
| pre-ab-build | Pre-A/B Build (paid-event telemetry, fossilize-rate guard, ON config, scoring pre-reg) | ready | high | M | medium | Plan `docs/plans/ready/2026-07-03-pre-ab-build.md` created 2026-07-03 (tracker esper-lite-3f3e9b9b4f, claimed); operationalizes gate criteria 3+4 conditions from the F2-freeze drl review under PDR-0019 scoring; WI-3 guard design (G1 in-run 5×-baseline 2-batch HARD abort / G2 scoring 3× paired-rate / G3 10% off-switch-J) in drl-expert review; OFF arm must stay bitwise-identical at scale=0; A/B launch is a separate owner GPU-go after this completes |
