...
| ID | Title | Type | Urgency | Complexity | Risk | Status |
|----|-------|------|---------|------------|------|--------|
| phase2-settlement-impl | Phase-2 Settlement (flag-on-HOLDING, default-OFF) + B-path replay | ready-pending-review | high | L | high | Created 2026-07-14; AUTHORIZED PDR-0090 (owner convergence-endorsement; scope: default-off impl + tests + replay + drl review + docs — NO training activation, NO GPU, NO obs changes); plan `docs/plans/ready/2026-07-14-phase2-settlement-implementation.md`; drl plan review dispatched; Filigree `esper-lite-57a56da421`; acceptance = pre-reg §2.6 seven gates + ten Phase-2 invariants + round-22 pins |
...
| entropy-floor-population-audit | Entropy-Floor Population Audit | concept | high | M | medium | Drafted 2026-07-10 from a 3-round external review (owner-relayed) + in-session code verification. Floor penalty averages availability-masked entropy incl. singleton zeros + forced steps → effective target depends on decision density; feasibility test d<floor; five-case classification; new per-head floor-population telemetry contract; objective change (if licensed) needs own PDR + experiment. Front-of-TIP candidate; audit runs post-freeze after the Stage-2 read. Common-mode for the live A/B (mediator, not confound). `docs/plans/concepts/2026-07-10-entropy-floor-population-audit.md` |
