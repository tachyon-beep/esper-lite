...
| reward-redesign-causal-run | Reward Redesign — Phase 0 / Causal-Contribution Run | concept | high | L | high | GATE −1 cheap-fix falsifier SURVIVED 2026-06-28 (25/25 scored, NO STOP — redesign not falsified; `esper-lite-a221da47ea`). Owner-gated causal-contribution run designed to resolve the (a)/(b) fork (freeloader vs LOO-undervalued enabling stem) + yield contribution-gate J: v1 (`docs/plans/concepts/2026-06-28-causal-contribution-run-design.md`) dual-reviewed → NEEDS REVISION; v2 (`…-v2.md`) authored 2026-06-29 via workflow + re-reviewed = **NEEDS-ANOTHER-PASS**. Design converged; 3 residual blockers are empirical/code: R1 RNG-pairing pilot (DISPOSITIVE — unsplit RNG can inject a bias making the primary Δ_struct CI falsely tight), R2 implement+re-review the §5 mask/latch/RNG diff, R3 confirm control GOVERNOR_ROLLBACK rows. **OWNER DECISION PENDING:** ratify v2 §0 total-system estimand pivot (deletes placebo arm, reduces Goal-1 to system-level dependence, can't name mechanism) OR commission a concrete placebo/DUMMY-R0C0 design to try to preserve the mechanistic-enabling claim. R1 pilot + RNG/mask impl are estimand-invariant — proceed either way. |
