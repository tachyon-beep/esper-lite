...
**Last Updated:** 2026-07-05 (Branch consolidation onto `main`: the product-ownership workspace, the Shapley/Phase-0/A-B line, and the squashed EV Stage-2 head (`575d2560`) merged to `main` to reduce divergent live branches; the EV granular work on `feat/ev-stab-stage2-hra` is reconciled separately, single-sourcing `partition.py`/`reward_variance.py`. Prior mainline state carried forward: PR #114, "Land 0.3.0 post-merge closeout line", merged 2026-06-21 at `b1d558fe` after the `0.3.0` head advanced to `3ad2e897`; PR #111, "Release 0.2.0: merge 0.1.1 -> main", merged 2026-06-19 at `d57ecf65`.) Previous: 2026-07-03b (`pin-e-placebo-harness` COMPLETED: tau=+0.28pp provisional lower bound delivered to the enablement gate; epsilon-plateau trigger fired — see PDR-0015 and the analysis doc.) Previous: 2026-07-03 (Added `pin-e-placebo-harness` to Tier 1: owner opened the Committed-Shapley enablement gate via its first leg (PDR-0014); plan drafted and under specialist review. Pointer correction: the PIN-E tracker task is esper-lite-94869250f1 — esper-lite-3d67b09687 was a mis-cite.) Previous: 2026-07-02 (Added `committed-shapley-topup-build` to Tier 1: owner build sign-off received; default-OFF reward-credit build planned and under specialist review.) Previous: 2026-06-21 (Main-merge reconciliation closed `esper-lite-569292a32b`: live GitHub shows PR #111, "Release 0.2.0: merge 0.1.1 -> main", merged on 2026-06-19 at `d57ecf65`, and current `origin/main` is `f8089677` with that release commit in history. The old `origin/0.1.1` source branch no longer exists, and local `backup/0.1.1-pre-p01` is not a valid source because it is behind `main` and still carries the old EV branch. The original 0.1.1 integration plan is therefore historical/completed and moved to `docs/plans/completed/2026-06-18-main-merge-integration-plan.md`. Separate follow-up task `esper-lite-224fdba503` now tracks landing or deliberately splitting the `0.3.0` post-merge closeout line, which remains 9 commits ahead of `origin/main` and contains the dependency-triage, EV-closeout, and Weft parity hardening commits.)
...
| committed-shapley-topup-build | Committed-Shapley Synergy Top-Up (default-OFF build) | completed | high | L | medium | COMPLETED 2026-07-02 same-day (owner sign-off → plan → drl+pytorch reviews approved-with-changes → TDD build → verification): 12 WIs across a479d776..2d7e0db5; ~100 new tests; reward-function-reviewer re-pass APPROVE_WITH_CHANGES (one change closed in-build); adversarial code review CLEAN; plan moved to `docs/plans/completed/2026-07-02-committed-shapley-topup-build.md`; flag stays `shapley_synergy_scale=0.0` — enablement is a separate owner-gated decision with banked criteria + build-time residuals (std-drift on credit divisor, cuDNN terminal-val_acc confound); tracker esper-lite-254175df90 closed |
| pre-ab-build | Pre-A/B Build (paid-event telemetry, fossilize-rate guard, ON config, scoring pre-reg) | completed | high | M | medium | COMPLETED 2026-07-03 same-day (drl plan review ADJUST→incorporated → TDD build bb8d3c90 → 4-lens adversarial review w/ double refutation: 28 agents, ONE MINOR survivor [ON-config pin test] fixed same-commit-cycle): payload provenance ext (running_std_raw/bound flags/v_table), G1 FossilizeRateGuard (≥13/batch ×2, emit-then-raise, scale>0-gated), normalized_cap≤REWARD_NORMALIZER_CLIP in both configs, ON config + pin tests, scoring pre-registration (criteria i–v, Δcorr floor, G2/G3/G4, asymmetric null, confound gate, tau recalibration); cleared both pre-existing tracked reds; suite green 5126+; plan → completed/; esper-lite-3f3e9b9b4f closed; REMAINING: owner GPU-go for paired n=5 launch |
| pin-e-placebo-harness | PIN-E Placebo Noise-Floor Harness (tau calibration) | completed | high | M | medium | COMPLETED 2026-07-03: k=3 redesign (PDR-0015) built WI-1..7 TDD (4d1e4c54..9be132c1); full 2-arm runs (3 seeds x std {1e-3,1e-4}) analyzed — non-degeneracy PASSED, epsilon plateau FAILED (floor is magnitude-dependent; PDR-0015 trigger fired) → tau=+0.28pp CONSERVATIVE-PROVISIONAL LOWER BOUND, mandatory first-ON-run recalibration. Analysis `docs/analysis/2026-07-03-pin-e-placebo-noise-floor.md`; plan moved to `docs/plans/completed/2026-07-03-pin-e-placebo-harness.md`; esper-lite-94869250f1 closed, tau memo on esper-lite-f22a1d48a7 |
| shapley-coverage-leg | Committed-Shapley Coverage Leg (future re-run design) | concept | low | M | low | ADDED 2026-07-05 after the n=5 A/B scored NULL-NOT-INFORMATIVE (coverage-bound; PDR-0026) and the owner PARKED the term (PDR-0027). `docs/plans/concepts/2026-07-05-shapley-coverage-leg-design-note.md` — binding facts (frequency-not-magnitude; P(pay\|k≥2)=44.7% vs 45.4% prior), anti-pattern (no scalar twiddling), licensing (≥10× PER-RUN paid-episode coverage; τ placebo legs incl. GATE-placebo before magnitude claims), event-level metrics for a sparse term (precision/recall vs J-positive structures; conditional-eligible corr; REJECTED the circular corr(top_up, excess)). Activates ONLY on PDR-0027 reversal triggers (organic k≥2 rise ≥3× from the EV track, or explicit owner reopen of path b/c). |
| reward-redesign-causal-run | Reward Redesign — Phase 0 / Causal-Contribution Run | ready | high | L | high | GATE −1 SURVIVED 2026-06-28 (25/25, NO STOP; `esper-lite-a221da47ea` closed). Estimand-invariant harness (SUPPRESS-SLOT + 3-domain RNG split + offset gates + NaN-safe patch) built, R2-reviewed, GPU-validated, **COMMITTED** (68fca06d). **R1 pilot ran (n=3, seeds 41–43).** The apparent "entropy collapse" was diagnosed (ultracode workflow wrpmqysf6 + verification) as a **MEASUREMENT ARTIFACT** — the anomaly detector read a forced-step-diluted per-head series; decision-step entropy is healthy (PDR-0006); telemetry read-path fixed (a84f9a78). **The (a)/(b)/(c) fork discriminator is J / acc-per-param, NOT accuracy** (accuracy ties): n=3 pilot = control 12–20 vs suppress 7–9 pp/M-param → suppressing r0c0 ~halves efficiency → leans **(b) efficiency-enabling stem** (NOT freeloader); banked as METHOD, not direction. **NEXT: n=5 J-read PRE-REGISTERED** — run sheet `docs/plans/ready/2026-07-01-n5-j-read-run-sheet.md`, analyzer `scripts/causal_contribution_j_analyze.py`; seeds 41–43 reused (`telemetry/causal_r1_n5/`) + 44–45 to run (~13h GPU). **OWNER-GATED:** Phase 1 GPU go; PDR-0004 estimand scope (total-system vs placebo/DUMMY). PDRs 0002–0007. |
