...
#### P3-CLONE — Stream-local data clone (drop per-batch default-stream `wait_stream`) **[DEFERRED 2026-06-14 — tracked]**
- **DECISION (2026-06-14): DEFERRED — not implemented this pass.** P3-CLONE removes the eager source-side clone (`data.py:590`) that is currently the bulletproof firewall against `nll_loss` wrong-target corruption, relocating it stream-local into already-intricate consumer stream choreography (`vectorized_trainer.py:588-650`). The reviewer rated the discipline correct but the VALUE LOW (drops one per-batch `wait_stream`) and the residual hazard is intermittent (allocator reuse of the freed source block — the exact corruption the eager clone prevents). The plan's own validation is *"run under load / multi-worker DataLoader and confirm no intermittent wrong-target corruption,"* which cannot be conclusively run headless in this pass. Per evidence-before-assertions, shipping a corruption-prone change without its mandated load-validation is the wrong trade against a small perf win, especially with the headline memory/throughput wins (P0 allocator, P2 stream pool, P1 BF16/sync removal) already landed and validated. **Left the working eager clone in place.** Tracked for a future pass with the load-corruption stress harness. The exact implementation (record_stream on raw views BEFORE the async stream-local clone — STOP-CLONE/R5) is preserved below.
