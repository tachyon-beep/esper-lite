...
- [x] Telemetry xfails (TELE-600–603) resolved; wiring tests now passing.
...
- Telemetry xfails for TELE-600–603 are resolved; no remaining xfails in this checklist.

## Execution notes (Phase 3 preflight)

- Full test suite: `UV_CACHE_DIR=.uv-cache uv run pytest`
  - 4257 passed, 34 skipped, 69 deselected, 0 xfailed
- Lint: `UV_CACHE_DIR=.uv-cache uv run ruff check src/ tests/` (clean)
- Types: `UV_CACHE_DIR=.uv-cache uv run mypy src/` (clean, 172 files)
...
- Throughput baseline run:
  ...
  - Run dir: `telemetry/telemetry_2026-01-07_001649`
  - Total episode time (TRAINING_STARTED → last EPOCH_COMPLETED): 38.879816s
  - Episodes/sec: 0.025720
  - Per-epoch deltas: [7.592749s, 7.516177s, 7.666476s, 7.449558s], avg 7.556240s
  - Phase 2 baseline for comparison: 44.754748s total, 0.022344 episodes/sec
- PBRS spike validation run:
  - Command: `PYTHONPATH=src UV_CACHE_DIR=.uv-cache uv run python -m esper.scripts.train ppo --preset cifar_baseline --task cifar_baseline --rounds 1 --envs 1 --episode-length 5 --telemetry-dir telemetry --device cpu --devices cpu --num-workers 0`
  - Run dir: `telemetry/telemetry_2026-01-07_001803`
