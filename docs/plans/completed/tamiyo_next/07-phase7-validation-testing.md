...
Fields like `gradient_ratio_ema` track gradient health over time. Verify the EMA updates correctly and no logic depends on stale values.

> **Note:** The `gradient_health_prev` feature (raw previous epoch gradient health) is planned
> but not yet implemented. The current codebase uses `gradient_ratio_ema` which tracks an
> exponential moving average. See Phase 2a½ for the planned field addition.

```python
def test_gradient_ratio_ema_field_updates():
    """Verify gradient_ratio_ema is actually updated, not stuck at initial value.

    BUG PATTERN: If the update logic has a bug, gradient_ratio_ema stays at 0.0
    ...
    assert state.gradient_ratio_ema == {}

    # After first epoch with a seed, should have entry
    slot_id = "slot_a"
    state.gradient_ratio_ema[slot_id] = 0.0  # Initial value

    # Simulate epochs with varying gradient ratios (EMA updates)
    gradient_ratio_sequence = [0.0, 0.1, 0.3, 0.5, 0.4, 0.2, 0.15]

    for epoch, current_ratio in enumerate(gradient_ratio_sequence[1:], start=1):
        prev_ema = state.gradient_ratio_ema.get(slot_id, 0.0)

        # If prev_ema is ALWAYS 0.0, the update logic is broken
        if epoch > 2:
            assert prev_ema != 0.0, (
                f"gradient_ratio_ema is stuck at 0.0 after {epoch} epochs. "
                f"The LSTM cannot learn gradient trends. "
                f"Check Phase 6b update logic in vectorized.py."
            )

        # Update for next epoch (simulating EMA update)
        state.gradient_ratio_ema[slot_id] = current_ratio

    # Verify the sequence was tracked
    assert state.gradient_ratio_ema[slot_id] == gradient_ratio_sequence[-1]
...
# REQUIRED: DEFAULT_MIN_FOSSILIZE_CONTRIBUTION must be defined in leyline
# Per CLAUDE.md 'No Bug-Hiding Patterns' policy, access directly - fail if missing
val = leyline.DEFAULT_MIN_FOSSILIZE_CONTRIBUTION  # Will fail if not defined
if val == 0.0:
    print(f'WARNING: DEFAULT_MIN_FOSSILIZE_CONTRIBUTION={val} may be too lenient')
else:
    print(f'DEFAULT_MIN_FOSSILIZE_CONTRIBUTION={val}: OK')
"
```
