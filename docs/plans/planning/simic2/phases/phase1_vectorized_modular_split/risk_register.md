...
- **Mitigation:**
  ...
  - Run the fast guardrail suite after each step (`docs/plans/planning/simic2/phases/phase0_baseline_and_tests/baseline_capture.md`, fast guardrail suite section).
...
- **Mitigation:**
  - Keep `esper.runtime.get_task_spec` lookup lazy (see `docs/plans/planning/simic2/phases/phase0_baseline_and_tests/baseline_capture.md`, import-cycle pressure points section).
...
### R5: Action execution contract drift (ad-hoc dict surfaces)

- **Severity:** Medium
- **Likelihood:** Medium
- **Where it bites:** `action_execution.py` → `vectorized_trainer.py` interface
- **Mitigation:**
  - Define a small explicit return structure for action execution results.
  - Pass mutable lists explicitly (no hidden side effects).
