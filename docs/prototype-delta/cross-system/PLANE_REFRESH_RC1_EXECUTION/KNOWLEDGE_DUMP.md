...
- **Async coordinator**: `KasminaPrefetchCoordinator` now prefers the shared `AsyncWorker` for publish/consume loops, tracks worker handles, and surfaces task failures through `poll_task_issue`. Shutdown cancels inflight handles and clears asyncio tasks deterministically.
...
- **Live Oona check**: Exercised redis-backed `KasminaPrefetchCoordinator` (single-loop mode) with 300 requests across 30 blueprints. Measured 0.30 s mean / 0.30 s median / 0.45 s max latency via telemetry (`kasmina.prefetch.latency_ms` reported ~1.09 s due to additional queueing). No cache contention observed. AsyncWorker version requires per-thread Redis clients to avoid cross-loop futures—log follow-up before enabling in production.
