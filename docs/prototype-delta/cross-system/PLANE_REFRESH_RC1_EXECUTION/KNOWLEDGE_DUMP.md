...
## Kasmina WP-K4 Baseline (2025-09-29)
- **Prefetch coordinator**: `KasminaPrefetchCoordinator` spawns two long-lived asyncio tasks (`_consume_ready`, `_consume_error`) using the active event loop; when no loop is running, `_schedule` falls back to `asyncio.run` unless an `AsyncWorker` is provided. Requests submitted through `_oona.publish_kernel_prefetch_request` are fire-and-forget—no telemetry or metrics capture queue depth, outstanding requests, or worker latency.
- **Lifecycle gaps**: `KasminaSeedManager` tracks outstanding requests in `_prefetch_requests`, but there is no cancellation on shutdown (`close()` is never invoked) and no guard against dangling requests when a seed is culled. `finalize_step` does not poll `KasminaPrefetchCoordinator.poll_task_issue`, so background task failures exit silently.
- **Cache contention**: Kernel attachment writes directly into `KasminaMemoryManager.kernel_cache` and the optional GPU cache without locking or single-writer enforcement. Concurrent prefetch completions could interleave attachments, and there are no telemetry counters for hits/misses beyond aggregate cache stats.
- **Testing coverage**: Only `tests/kasmina/test_prewarm.py` exercises the prewarm path. No unit or integration tests assert prefetch request flows, error handling, cancellation, or cache locking behaviour. Integration control loop runs without the coordinator active in most fixtures (prefetch disabled via timeout hacks).

## Kasmina WP-K4 Progress (2025-09-29)
- **Async coordinator**: `KasminaPrefetchCoordinator` now prefers the shared `AsyncWorker` for publish/consume loops, spawns per-task Oona clients (unique Redis connections), tracks worker handles, and surfaces task failures through `poll_task_issue`. Shutdown cancels inflight handles, closes spawned clients, and clears asyncio tasks deterministically.
- **Prefetch metrics & timeouts**: `KasminaSeedManager` records per-status counters (`kasmina.prefetch.requests_total{status}`), inflight depth, and latency (average + last sample). Outstanding requests carry issued timestamps; `finalize_step` expires entries that exceed `prefetch_timeout_ms` (~2× latency budget) and emits CRITICAL `prefetch_timeout` telemetry with gate failures. Unit tests cover ready/error/timeout/cancel flows.
- **Cache locking**: Kernel attachment now acquires per-blueprint locks around memory/GPU cache mutation, recording wait time via `kasmina.cache.lock_wait_ms` and WARNING `cache_lock_contention` events when contention exceeds 100 ms. Prefetch-ready/resume paths include lock instrumentation.
- **Reset hygiene**: Registry/teacher resets and seed retirement cancel outstanding requests, decrement inflight counters, and emit `prefetch_canceled` telemetry so operators can distinguish administrative cleanups from timeouts.
- **Benchmark**: Synthetic run (200 seed grafts, 2 ms kernel fetch, 0–6 ms jitter) yielded 0.56 s mean / 0.58 s median / 1.09 s max prefetch latency with zero cache contention. Adopt alert thresholds of 1.2 s (warning) and 1.5 s (critical) on `kasmina.prefetch.latency_ms`; `kasmina.cache.lock_wait_ms` remained <1 ms.
- **Live Oona check**: Exercised redis-backed `KasminaPrefetchCoordinator` (single-loop mode) with 300 requests across 30 blueprints. Measured 0.30 s mean / 0.30 s median / 0.45 s max latency via telemetry (`kasmina.prefetch.latency_ms` reported ~1.09 s due to additional queueing). No cache contention observed. AsyncWorker path now spawns dedicated Redis clients per task, eliminating cross-loop future warnings.
- **Outstanding**: Integrate the benchmark thresholds into Grafana dashboards/alert rules and document the async worker enablement steps in the observability runbook.
