# Current State — Esper        Checkpoint: 2026-07-03 (checkpoint #10 — pre-A/B build ACCEPTED; paired n=5 OFF/ON A/B LAUNCHED on owner GPU-go; PDR-0020, PDR-0021)
...
## In flight
- **Paired n=5 OFF/ON A/B on both GPUs** (owner GPU-go 2026-07-03 "roll straight into
  the test"): seeds 41–45, both arms at the launch commit (recorded in the scoring
  pre-registration doc), telemetry → telemetry/shapley_ab_n5/{off,on}_s*. OFF wave then
  ON wave per GPU. G1 guard armed on ON runs only. NOTE: the old causal_r1_n5 controls
  are at an older commit and are NOT the OFF arm (PDR-0015 same-commit rule).
...
## Last checkpoint did (checkpoint #10)
...
- Build shipped (bb8d3c90+459d01cf): payload provenance + v(S) table, G1 guard,
  ...
  plan → completed/.
- Launched the A/B (this checkpoint's DISPATCH tail, owner-directed).
