...
| **EV Stage-0 gate** — `Cov(R_cf, R)/Var(R)` on a Stage-2-**OFF** control run (value-free, raw-scale, per-return decomposition) — THE Now-bet metric | **`> 0.40` ⇒ cf stream dominates return variance ⇒ Stage-2 de-shaping justified; AND low `r_main_var_share = Var(R_main)/Var(R)` (the smoothness leg)** | **NO VALID READING YET** — gate not wired. Core math landed on `feat/ev-stab-stage2-hra` (`compute_return_variance_shares`, TDD 4/4 green). The existing ON-leg metric is V_cf-contaminated and **invalid for the gate** (PDR-0028); reading requires the per-component buffer SoA + both-legs wiring (esper-lite-3d67b09687) | 2026-07-05 (pending) |
