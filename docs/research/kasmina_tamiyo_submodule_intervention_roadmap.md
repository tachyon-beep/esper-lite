...
| **C5: addressable subtargets (inventory)** | New head `subtarget_idx` + enable/disable ops | Selective pruning within a seed | Debuggable micro-decisions; sets up Phase α scaling | Requires new action head + masks; increases learnability burden | Keep K small (≤8–16) and grouped; token policy likely required |
...
| **3 (A + C inventory)** | 3–4w | Phases 1–2 | High | Subtarget ops used with low invalid-action rate; effective params reduced at same quality; no persistent entropy collapse in new head |
...
- Tamiyo chooses `(slot, op, subtarget_idx)` where `op ∈ {ENABLE_SUBTARGET, DISABLE_SUBTARGET}`.
- Subtarget IDs are stable integers `0..subtarget_count-1` determined at germination.
