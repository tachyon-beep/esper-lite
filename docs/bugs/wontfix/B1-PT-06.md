...
Ticket incorrectly claimed signature already accepted `torch.device | str`. Actual signature is `device: str = "cuda"` (str only). P4 polish with trivial workaround (`str(device)` at call sites). Not worth the churn.
