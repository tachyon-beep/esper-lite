...
CodeReview agent objects:
1. **Adds complexity** — Returning `SnapshotInfo` changes API for marginal value
2. **No current use case** — Callers don't need snapshot metadata
3. **Telemetry is better** — If observability is needed, emit events (Option 3)

Current void return is appropriate.
