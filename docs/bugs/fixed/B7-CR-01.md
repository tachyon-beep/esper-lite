...
Implemented **Option 1** as recommended by all three specialists:

1. **Removed trap defaults** from `check_value_function()` - now `current_episode: int` and `total_episodes: int` are required arguments (no `= 0`)

2. **Removed trap defaults** from `check_all()` - same parameters now required, reordered to come before optional `has_nan`/`has_inf`

3. **Fixed misleading docstring** - removed "(0 = use static threshold)" claim that contradicted the implementation

4. **Updated test** - `test_requires_episode_info_for_value_collapse_thresholds` now expects `TypeError` (missing required arg) instead of `ValueError` (trap default rejected)

### Files Changed

- `src/esper/simic/telemetry/anomaly_detector.py`: Lines 123-142 (signature + docstring), Lines 297-324 (signature + docstring)
- `tests/simic/test_anomaly_detector.py`: Lines 93-101 (updated test)

### Verification

- [x] All 16 anomaly_detector tests pass
- [x] mypy clean on modified files
- [x] All production callers already pass required arguments

### Commits

- `c418809b` fix(simic/telemetry): B7-CR-01 - remove trap defaults from AnomalyDetector
