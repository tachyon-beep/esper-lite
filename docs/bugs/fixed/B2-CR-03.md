...
**Evidence:** The documentation requested by this ticket already exists in `src/esper/kasmina/slot.py`:

1. **Module docstring (lines 21-37)**: "DDP Symmetry Requirements" section explicitly states all ranks must call `advance_stage()` / `step_epoch()` in identical order, with consequences documented.

2. **Method docstring (lines 2202-2204)**: "COLLECTIVE OPERATION: All ranks MUST call this in identical order for each slot's gate check, or deadlock will occur."

The "COLLECTIVE OPERATION: All ranks MUST" language satisfies the ticket's request for an INVARIANT comment. Optional debug logging deferred until actual DDP ordering issues arise.

**Sign-off:** Confirmed by `feature-dev:code-reviewer`
