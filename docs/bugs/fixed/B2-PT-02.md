...
**Option A implemented with helper function.**

#### Investigation Findings

| Claim | Status | Evidence |
|-------|--------|----------|
| "Unconditional .to() adds overhead" | ✅ TRUE | Benchmarked: conditional check is ~7x faster per call |
| "PyTorch short-circuits same-dtype" | ✅ PARTIAL | Same object returned (no allocation), but function call overhead exists |
| "torch.lerp handles mixed dtypes" | ❌ FALSE | torch.lerp REQUIRES matching dtypes - fails with "expected dtype float for `end`" |
| "Conversion is required for correctness" | ✅ TRUE | torch.lerp fails under mixed precision without dtype alignment |

#### Fix Applied

Added `_to_dtype_if_needed()` helper function that conditionally converts:
...
Updated all three blend functions:
- `blend_add()`: Lines 53-54
- `blend_multiply()`: Lines 100-103
- `blend_gate()`: Lines 134-135

#### Verification

- [x] Benchmark confirms conditional check is faster
- [x] All 16 blend_ops tests pass (contracts + gradients)
- [x] torch.compile fullgraph tests pass
- [x] Correctness preserved for mixed-precision training
