...
The ticket only examines the **terminal bonus** in isolation, missing that the **immediate fossilize reward** already scales with contribution:

| Reward Component | 1% Seed | 10% Seed | Formula |
|------------------|---------|----------|---------|
| Immediate fossilize | 0.6 | 1.5 | `0.5 + 0.1 * contribution` |
| Terminal bonus | 3.0 | 3.0 | `3.0 * count` (flat per seed) |
| **Total** | **3.6** | **4.5** | |

A 10% contributor receives **25% more total reward** than a 1% contributor.

#### The Two-Component Design Is Intentional

1. **Immediate shaping rewards QUALITY** (`_contribution_fossilize_shaping()`, lines 1331-1337):
   ```python
   base_bonus = config.fossilize_base_bonus + config.fossilize_contribution_scale * seed_contribution
   ```
   Higher-value seeds get higher immediate rewards.

2. **Terminal bonus rewards QUANTITY** (line 805):
   ```python
   fossilize_terminal_bonus = num_contributing_fossilized * config.fossilize_terminal_scale
   ```
   Encourages fossilizing multiple valuable seeds, with the 1% threshold filtering out worthless ones.

#### Documented Design Philosophy

From `rewards.py` lines 95-96:
```
# - FOSSILIZED (6.0): +0.5 (SMALLEST delta) - terminal bonus
#   Small to prevent "fossilization farming" (rushing to completion)
```

The design explicitly prevents "fossilization farming" by keeping the terminal bonus flat while providing scaled rewards through immediate shaping.

#### Why The Proposed Fix Would Be Harmful

Scaling terminal bonus proportionally would:
1. **Double-count contribution** - immediate shaping already scales with it
2. **Create compounding incentives** - high-value seeds would get disproportionate reward
3. **Reduce quantity incentive** - policy might fossilize fewer, higher-value seeds instead of multiple valuable ones

#### Severity Downgrade

- Original: P2 (based on incomplete reward system analysis)
- Revised: P4 (misunderstanding of intentional design)
