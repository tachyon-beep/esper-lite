...
# Input
features: [batch, seq_len, 120]   # Obs V3 non-blueprint (24 base + 32 per slot * 3)
...
```python
# Total non-blueprint dims: 24 base + 32 per-slot * num_slots (120 for 3 slots)
# Blueprint identity moved to embeddings inside the network (4 dims per slot).

# Base features (24 dims): tamiyo/policy/features.py
...
- stable_val_acc_for_observation (1)

# Per-slot features (32 dims each)
